...
;			print,"[dim,ed,imin,i,imax]=",[dim,ed,imin,i,imax]
			...
			wherefoot=where(footI[imin:i] ne 0)
			foot=wherefoot[-1]
			if foot eq -1 then foot=imin
...
;			if dhalf gt i or imin gt dim then dhalf= imin
			...
			print,"[dim,ed,imin,i,imax]=",[dim,ed,imin,i,imax]
			while footI[ii] ne 0  and ii ge imin do begin
			...
			if i ne ii then begin
				...
				if (ii-iii gt 60)  or fore[ii] eq foremax[ii] then begin
						if ii gt 30+ed and fore[ii] eq foremax[ii] then begin 
							i2=ii
							while ii ge bound+60 and (stddev(B[ii-60,ii]) ge 1.5 or ((stddev([B[ii-60],B[ii],mean(B[ii-60:ii])]) lt .5  ) and (max(B[ii-60:ii])-mean(B[ii-60:ii]) gt 2)   )) do ii--;while (ii ge ed+61) and (i2-ii lt 240) do ii--;and ((B[ii]-B[ii-60] ge 1.5) or ((stddev([B[ii-60],B[ii],mean(B[ii-60,ii])]) lt .15  ) and (max(B[ii-60:ii])-mean(B[ii-60,ii]) gt 2)   )) do ii--			
							Bmax=max(B[ii-60:ii],maxuploc)
							if Bmax ge indowns[i]-1 or total(abs(B[ii:60:ii]-indowns[i]) lt abs(B[ii-60:ii]-inups[i])) gt 5 then begin
								...
								if AB[0]-ii+60 lt 57 or total(B[bound:AB[0]-1] ge indowns[i]-1)/(-bound+AB[0]-1) gt .7 then begin
									...
									while(max(B[ii-60:ii],maxuploc) ge indowns[ishock]-1 and ii-60 gt bound) do ii--

									
								endelse
								if abs(Bminute[imin+30]-up) lt .2 and max(B[imin:imin+60]) lt mean([indowns[i],inups[i]]) and abs(b60d[imin+30]) lt .2 then begin
									st=imin
									en=imin+60
									print,"[imin,i,bi,st,en,imax,ed,dim]=",[imin,i,bi,st,en,imax,ed,dim]
									for kk=i-10,imax do begin;for kk=imin,imax do begin
...
imin=min([omaxs[el],omins[el]])
				...
				if (iii-ii gt 60)  or fore[ii] eq foremax[ii] then begin
					...
					;	uendini[ii]=min([ii+60,N-1])
						if ii  lt ed-60 and fore[ii] eq foremax[ii] then begin 
								...
								if Bmax ge indowns[i]-1 or total(abs(B[ii:ii+60]-indowns[i]) lt abs(B[ii:ii+60]-inups[i])) gt 5 then begin
								;if max(B[ii:ii+60],maxuploc) ge outdowns[i]-1 then begin
									AB=where(B[ii:ii+60] ge outdowns[i]-1)+ii
									if -AB[-1]+ii-60 lt 57 or total(B[AB[-1]+1:bound] ge outdowns[i]-1)/(-AB[-1]-1+bound) gt .7 then begin
										...
										while(max(B[ii:ii+60],maxuploc) ge outdown[ishock]-1 and ii+60 lt bound) do ii++
									endelse

									if abs(Bminute[imax-30]-up) lt .2 and max(B[imax-60:imax]) lt mean([indowns[i],inups[i]]) and abs(b60d[imax-30]) lt .2 then begin
									st=imax-60
									en=imax
									print,"[imin,i,bi,st,en,imax,ed,dim]=",[imin,i,bi,st,en,imax,ed,dim]
									for kk=imin,i+10 do begin;for kk=imin,imax do begin
	...
	;					uendini[kk]=min([ii+60,N-1])
			...
			bi=imax-29;(where(abs(xs-rxf) eq min(abs(xs-rxf))))[0] ; this is the index of the closest time value in the B frame (where dt=1 sec)
