...
curveType='expofit'
MM=[(2-.25)/(18-1),1,1,.25]
MM=[ 0.109218 ,0.671033  ,0.919866,     0.283866]
ASMOOTH=smooth(OA0,30)
expofit,MA0,MM,F
;p3=plot(MA0,F,'g-')
yfit=curvefit(MA0, OA0, weight, MM, SIGMA, FUNCTION_NAME='expofit',status=status,CHISQ=CHISQ)
help,yfit

;p1=scatterplot(MA0,ASMOOTH,sym_color='g',/over)
;p1=scatterplot(MA0,OA0,/over)
;p2=plot(MA0,yfit,'b-',/over)
;p1=scatterplot(MA0,ASMOOTH,sym_color='g',/over)
;p4=scatterplot(MA0,OA0/yfit)

tsort=sort(t0)
yfit=yfit[tsort]
print,total(MA-MA0[tsort])
print,total(OA-OA0[tsort])
;p5=scatterplot(MA,OA/yfit)
print,MM
print,CHISQ
...
store_data,'overshootAmplitudeVsMA',data={x: t ,y: AM ,ytitle:[ '(($B_{Max} - B^{Fit}_{Down}$)/$B^{Fit}_{Down}$)$\n$ ----------------------$\n$ $(.11(M_{fms}/M_{crit}-.67)^{.92}+.28)$' ], YN:[ "$M_{fms}/M_{crit}$ normalized A" ], fn:[ "Normalized_OvershootHeightvMA" ], binsize:[ .1 ], radian:[0],degree:[0]}

	get_data,'overshootAmplitudeVsMA',data=datAM
	...
	Title=subT+YT+" vs "+XT
	p1pos=[0.11562500  ,    0.62109375   ,   0.9   ,   0.90117187]
	if 0 then begin
	...
	Y=daty.y
	XT=datx.YN[0]
	YT="Overshoot Amplitude";daty.YN[0]
	yttl=daty.ytitle[0]
	xttl=datx.ytitle[0]
	...
	p1=scatterplot(X,Y,xtitle=xttl,ytitle=yttl,title=Title,SYMBOL='.', /SYM_FILLED,sym_size=.1,layout=[2,2,3],/curr,pos=p1pos)
	...
	for KK=0,3 do begin
		...
		plotlist1.add,scatterplot(xsub,ysub,sym_size=.05,$
				SYMBOL='.', /SYM_FILLED,POSITION=pltpos, $
				sym_color=seasonColors2[KK],Name=seasonNames[KK],/overplot)
	endfor
	medliner,X,Y,binsize,mlow,mhigh,medovershoot,errorbars
	plotlist1.add,errorplot((mlow+mhigh)/2.0, medovershoot,errorbars,color=!color.slate_grey,name="Median",/overplot,thick=.5)
	...
	(plotlist2[-1]).save,dire+'MALSplots.eps'
	print,pos3
	p1.close
	return
	;;;ZOOMED IN for abs(AM) lt 100
...
seasonsubIndices=list(intersect(springIndices,W),intersect(summerIndices,W),$	
	...
	(plotlist2[-1]).save,dire+'CritRatPlots.eps'
	print,pos4
