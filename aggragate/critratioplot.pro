...
weight=1.0/OA0
curveType='expofit'
MM=[(1.5-.1)/(6-.25),.25,1,0.1]
expofit,FM0,MM,F
;p3=plot(FM0,F,'g-')
yfit=curvefit(FM0, OA0, weight, MM, SIGMA, FUNCTION_NAME='expofit',status=status,CHISQ=CHISQ)
help,yfit
;p1=scatterplot(FM0,OA0,/over)
;p2=plot(FM0,yfit,'b-',/over)
;p4=scatterplot(FM0,OA0/yfit)
tsort=sort(t0)
yfit=yfit[tsort]
print,total(FM-FM0[tsort])
print,total(OA-OA0[tsort])
;p5=scatterplot(FM,OA/yfit)
print,MM
print,CHISQ
...
store_data,'overshootAmplitudeVsFM',data={x: t ,y: AM ,ytitle:[ '(($B_{Max} - B^{Fit}_{Down}$)/$B^{Fit}_{Down}$)$\n$ ----------------------$\n$ $(.21(M_{fms}/M_{crit}+.24)^{.84}+.146)$' ], YN:[ "$M_{fms}/M_{crit}$ normalized A" ], fn:[ "Normalized_OvershootHeightvFM" ], binsize:[ .1 ], radian:[0],degree:[0]}

	get_data,'overshootAmplitudeVsFM',data=datAM
