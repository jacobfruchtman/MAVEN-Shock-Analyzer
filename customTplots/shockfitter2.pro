...
pro shockFitter, newName=newName,curveType=curveType

	g0=0
	if NOT KEYWORD_SET(newName) THEN newName='B_Fitted'
	...
	;------------------
		...
		for i=1,gn-1  do begin
			...
			yBSbeg=yBS[gim:hj] ; elements before the first trigger. to zeroth order, this is (probably) before the step

			yBSend=yBS[hj:gi] ; elements after the first trigger. to zeroth order, this is (probably) after the step starts. ish 

			yBSbegtrust=ybeg[2 *nbeg/5: 4*nbeg/5] ; we're pretty sure the step will never be  in this region
			;mbeg=nbeg
			;mend=nend
			yBSendtrust=yend[2 * nend/5: 4*nend/5] ; we're pretty sure the step will always be on in this region
			
		

			stdTbeg=stddev(yBSbegtrust)

			stdTend=stddev(yBSendtrust)

			meanTbeg=mean(yBSbegtrust)

			meanTend=mean(yBSendtrust)

			meanTrust=mean([meanTbeg,meanTend])


			print,"meanTbeg= ",meanTbeg,", stdTbeg= ",stdTbeg
			print,"meanTend= ",meanTend,", stdTBend= ",stdTend
			print,"meanTrust= ",meanTrust, "ypBS[hj-gim]=",ypBS[hj-gim]

			BSh=ypBS[hj-gim]

			hosc=[0,0]
			hhj=hj
			while ((meanTrust+stdTend lt BSh) or (meanTrust-stdTbeg gt BSh)) do begin

				if(meanTrust+stdTend lt BSh) then begin
					hhj--
					BSh=yBS[hhj]
					hosc[1]=1
				endif

				if(meanTrust-stdTbeg gt BSh) then begin
		...
		;	while((ybeg[0] gt mean(yendtrust)) or (yend[nend-1] lt mean(ybegtrust))) do begin 
			...
			yBSbeg=yBS[gim:h0] ; elements before the first trigger. to zeroth order, this is (probably) before the step

			yBSend=yBS[h0:g0] ; elements after the first trigger. to zeroth order, this is (probably) after the step starts. ish 

			yBSbegtrust=yBSbeg[2 *nbeg/5: 4*nbeg/5] ; we're pretty sure the step will never be  in this region
			;mbeg=nbeg
			;mend=nend
			yBSendtrust=yBSend[2 * nend/5: 4*nend/5] ; we're pretty sure the step will always be on in this region
