pro zerothbow,keep_plot=keep_plot
	...
	if size(datPos,/typ) eq 2 then begin
...
;  options,'wind_interpolated','color','blue'
	...
	th2=atan(y,z)
	n_conic=fltarr(N,3)
	for i=0,N-1 do n_conic[i,0]=snxx[i]
	for i=0,N-1 do n_conic[i,1]=snrr[i]*cos(th2[i])
	for i=0,N-1 do n_conic[i,2]=snrr[i]*sin(th2[i])
	;n_conic=fltarr(N,3)
NN=fltarr(N)
for i=0,N-1 do NN[i]=Sqrt(total(n_conic[i,*]^2))
for i=0,N-1 do n_conic[i,0]=n_conic[i,0]/NN[i]
for i=0,N-1 do n_conic[i,1]=n_conic[i,1]/NN[i]
for i=0,N-1 do n_conic[i,2]=n_conic[i,2]/NN[i]

store_data,'n_conic',data={x:time,y:n_conic,ytitle:"Conic normal",LABELS:['x','y','z'],LABFLAG:1}

tplot_element,'mvn_B_1sec_MAVEN_MSO','y',Bvec

Bconic=dotproduct(Bvec,n_conic)
store_data,'B_conicN',data={x:time,y:Bconic,ytitle:'Conic direction Magnetic Field [nT]'}
tplot_element,'mvn_B_1sec_MAVEN_MSO_Mag','y',Bmag
thBC=acos(Bconic/Bmag)
store_data,'thetaBconicRad',data={x:time,y:thBC,ytitle:'Conic Magnetic Field angle [rad]'}
thBCd=90-abs(90-180*thBC/!pi)
store_data,'thetaBconic',data={x:time,y:thBCd,ytitle:'Conic Magnetic Field angle [deg]'}
