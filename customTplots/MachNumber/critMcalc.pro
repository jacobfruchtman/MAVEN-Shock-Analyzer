Pro critMcalc,angle=angle,startdate=startdate,currtime=currtime
	...
	;if total(datins.y + datouts.y) eq 0 then return
	xs=datB.x
	x0=xs[0]
...
;	get_data,'alfven_speed_Fine',data=datVA



	get_data,'Plasma_Beta',data=datbeta

	if not keyword_set(angle) then angle="Shock_Angle_AVG";best";AVG"
...
;
	...
	;crits=Cs*0.0
	crits=fltarr(N)
	;for i=0, numel(crits)-1 do crits[i]=0.0 
	...
	for el=0,gcount-1   do begin
		...
		;iup=max([i-20*60,0])
		;print,"iup=",iup
		betas2[i]=mean(betas[ubeg:uend],/nan);betas[iup]
		...
		crits[i]=calccritmachnumber(th1,betas2[i]);crit
