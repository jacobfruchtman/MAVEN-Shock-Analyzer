function findnextovershootmax, arr, B, down,wid
	wh=.5*.75*wid
	N=numel(arr)
	sp=0
	atP=1
	...
	for i=1, N-30 do begin;2 do begin
		if (arr[i] gt arr[i+1]) and (arr[i-1] gt arr[i]) and B[i] gt down and i gt wh then begin
		...
		if (arr[i] gt arr[i+1]) and atP and max(B[sp:i]) gt down and i gt wh then begin
