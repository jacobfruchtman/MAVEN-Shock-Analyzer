...
pro curveFitter2, plt,newName=newName,curveType=curveType
	...
	store_data,'infmaybe',data={x:datin.x,y:inf}
	...
	;------------------INBOUND DIRECTION -----------------------------------------------------------------INBOUND DIRECTION-----------------
		...
		Gl = WHERE(oyde ne 0, gn, COMPLEMENT=G_C, NCOMPLEMENT=gcount_c)
		

		Hd = WHERE(oydb ne 0, hn, COMPLEMENT=H_C, NCOMPLEMENT=hcount_c)
		print,"Size(Gl)=",size(Gl)
		print,"Size(Hd)=",size(Hd)

		;print,"oxde[Gl]-oxde[0]=",oxde[GG]-oxdb[0]
		;print,"oxdb[Hd]-oxdb[0]=",oxdb[HH]-oxdb[0]
		gx=oxde[Gl]
		hx=oxdb[Hd]
		gxa=gx-oxde[0]
		hxa=hx-oxdb[0]
		...
		for i=0, N-2 do begin
				for j=0, hn-1 do begin
					if ((hxa[j] le xsa[i]+xsa[1]) and (hxa[j] ge xsa[i+1]-xsa[1])) then begin
						...
						ydb[i+1]=1.0*(hxa[j] gt a)
						ydb[i]=1.0*(hxa[j] le a)
						print,"ydb[i]=",ydb[i],", ydb[i+1]=",ydb[i+1]
					endif
				endfor
		endfor
	...
	;		if(total(oxde[Gl] eq xs[i]) eq 1) then yde[i]=1
	;		if(total(oxdb[Hd] eq xs[i]) eq 1) then ydb[i]=1
	;	endfor
		GG = WHERE(yde ne 0, gn, COMPLEMENT=G_C, NCOMPLEMENT=gcount_c)
		print,yde[GG]

		HH = WHERE(ydb ne 0, hn, COMPLEMENT=H_C, NCOMPLEMENT=hcount_c)
	...
	;------------------
		...
		;trust=0
		g0=0
		;GG = WHERE(oyde ne 0, gn, COMPLEMENT=G_C, NCOMPLEMENT=gcount_c)
		

		;HH = WHERE(oydb ne 0, hn, COMPLEMENT=H_C, NCOMPLEMENT=hcount_c)
...
;
			...
			while((meanT+stdend lt BSh) or (meanT-stdbeg gt BSh)) do begin
				...
				;	break 
				;endif
				if((midij ge gi) or (midij le gim)) then begin
					;hhj+=hosc[1]-hosc[0]
					midij=mij
					break 
				endif
			...
			print,"mij=",mij,", gim=",gim, ",gi=",gi,", size(xpa,/n_el)=",size(xpa,/n_el)
			mij=midij
			print,"mij=",mij,", gim=",gim, ",gi=",gi,", size(xpa,/n_el)=",size(xpa,/n_el)
			...
			ishock=where(round(a2 mod xpa) eq 0)+gim
		...
		if(bcutoff eq 0) then begin
			...
			yendtrust=yend[8* nend/10: 9 * nend/10] ; we're pretty sure the step will always be on in this region
			brefines=0
			frefines=0
		...
		;	while((ybeg[0] gt mean(yendtrust)) or (yend[nend-1] lt mean(ybegtrust))) do begin 
			...
			while  (((ybeg[0] gt mean(yendtrust)) and incfunctions) or ((yend[nend-1] lt mean(ybegtrust)) and decfunctions  )) do begin 
				...
				;we will do these seperately within the loop, but at same time so that one negligably affects the other 
				if ((ybeg[0] gt mean(yendtrust)) and (incfunctions eq 1)) then begin; this is a simple approximation, but checking if on previous step
					print,"pushing forward the end of shock number i=",0,", from gim=",gim,", where h0=",h0,", and g0=",g0, "and size(yp)=",size(yp,/n_el)
					yp=yp[1:*] ;shrinks our range by one
 					ybeg=ybeg[1:*]
					np--
					nbeg--
			...
			while((meanT+stdend lt BSh) or (meanT-stdbeg gt BSh)) do begin
				...
				;	break 
				;endif
				if((midi0 ge gi) or (midi0 le gim)) then begin
					;hhj+=hosc[1]-hosc[0]
					midi0=mi0
					break 
				endif
			...
			print,"mi0=",mi0,", gim=",gim, ",gi=",gi,", size(xpa,/n_el)=",size(xpa,/n_el)
			mi0=midi0
			print,"mi0=",mi0,", gim=",gim, ",gi=",gi,", size(xpa,/n_el)=",size(xpa,/n_el)
		...
		;	print,"to zeroth order, guess that AA=",AA
			...
			yfit=CURVEFIT(xpa, yp, weight, AA, SIGMA, FUNCTION_NAME=curveType)
			PRINT, 'Function parameters: ', AA
			ishock=where(round(a2 mod xpa) eq 0)
