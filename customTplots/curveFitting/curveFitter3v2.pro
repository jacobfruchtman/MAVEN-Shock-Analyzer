...
pro curveFitter3, plt , newName=newName, curveType=curveType , doRoll=doRoll, doSecond=doSecond,Debug=Debug,minChi=minChi,fitnum=fitnum,secondOrder=secondOrder,doThird=doThird,orderCustom=orderCustom,minM0=minM0
...
print,'=3==3==3==3==3==3==3==3==3==3==3==3==3==3==3==3==3==3==3==3==3==3==3==3='
		...
		for i=1,fullIter-1  do begin
			...
			foot=(where(ifoots lt aj and ifoots ge dim))[0]
			
			if foot eq -1 then foot=dim+30
			t=tau[aj]
...
;
			...
			aj=ajRefiner(dim,aj,bi,zeron,dobug,yBS,nbeg,nend, mono,hill,hill2,orderCustom,extr,rhod,Fore)
...
;			m2=xpa[hj-gim]
			...
			fracRemain=(bi-ishock)*1.0/(bi-dim)
				badsl=0
				if ishock gt bi or ishock lt dim then begin
 ...
 and (array_equal(hill2e[0:min([60,imax-ishock])], hill2e[SORT(hill2e[0:min([60,imax-ishock])])]) )) 
			...
			working=1*( ((status eq 0) and (MM[1] gt 0)) and (CHISQ lt minChi) and (bi-ishock gt minRemain) and ( MM[0] gt minM0) and (ishock gt imin) and (ishock lt imax) and (abs(MM[3]-MM[0]-mn) lt maxUp) and (MM[1] gt minM1) and (CHISQ gt 0)) and (Fore[ishock] eq 1)  and (abs(yfit[0]-BU[ishock]) lt upDev) and ~pointerinup and ~badsl and total(yfit) gt 0;(TOTAL(Bmed[max([ishock-10,0]):min([ishock+2*60,imax])] gt MM[0]+MM[3]) ne 0)
			if doPerturb then begin
				...
				while yBS[imin]-yBS[dim] gt 10 do begin
					imin--
				;ishock--
				;imax--
				endwhile
...
;------------------------------END OF STANDARD RANGE--------------;------------------------------END OF STANDARD RANGE--------------;------------------------------END OF STANDARD RANGE--------------
		...
		if(aoffsetf eq 0) and ~error_status then begin
...
;			dim=0
			...
			if foot ne d00 then mn=BU[foot] else mn=BU[a0];mean(ybegtrust)
			dim=bfoot;(bfoot*3+dim)/4;mean([bfoot,bfoot,bfoot,bfoot,bfoot,dim])
			;print,'g0=',b0
			zn=z[b0:*]
			regp=reg[dim:b0]
...
;			if (rollsec eq 1) then a0=secondRoll(a0,bi,hill,dobug) ; second order correction
			...
			a0=ajRefiner(dim,a0,bi,zeron,dobug,yBS,nbeg,nend, mono,hill,hill2,orderCustom,extr,rhod,Fore)
			ab=(a0+bi+bi)/3
			while ab lt bi and max(ymm[a0:ab])+5 lt max(ymm[ab:bi],abmax) do bi=abmax+ab-1
 ...
 and (array_equal(hill2e[0:min([60,imax-ishock])], hill2e[SORT(hill2e[0:min([60,imax-ishock])])]) )) 
			...
			working=1*(((status eq 0) and (MM[0] gt minM0)) and (CHISQ lt minChi) and (bi-ishock gt minRemain) and $
			( MM[0] gt 1) and (ishock gt imin) and (ishock lt imax) and  $
			...
			if doPerturb then begin
				...
				mn=min(smmp)
				if MM[3]-MM[0]  lt min(ypp[0:8*(ishock-imin)/10])  or MM[3]-MM[0]  gt max(ypp[0:8*(ishock-imin)/10]) or total(MM[3]-MM[0] gt ypp[0:8*(ishock-imin)/10]) lt 5 or total(MM[3]-MM[0] lt ypp[0:8*(ishock-imin)/10]) lt 5 then begin;if MM[3]-MM[0]  lt
			...
			;	smypp=(smooth(ypp,2,/edge_mirror))[0:8*(ishock-imin)/10]
				...
				ishock=round(MM[2]/MM[1]) +imin
				;derv=dir(yfit,1,'mid')
				print,"ishock,imax=",ishock,imax,numel(hill2)
				badsl=0
				if ishock gt imax or ishock lt imin then begin
					 ishock=ishock1;imax=min([bi,ishock+7*60])
					badsl=1
				endif
				print,"dim,imin,ishock,imax,bi=",dim,imin,ishock,imax,bi,numel(hill2)
				hill2e=hill2[ishock:max([imax,ishock+151])]
 ...
 and (array_equal(hill2e[0:min([60,max([imax,ishock+61])-ishock])], hill2e[SORT(hill2e[0:min([60,max([imax,ishock+61])-ishock])])]) )) 


				working1=working
				working=1*(((status eq 0) and (MM[0] gt 1)) and (CHISQ lt minChi) and (bi-ishock gt minRemain) and ( MM[0] gt minM0) and (ishock gt imin) and (ishock lt imax) and  (MM[3]-MM[0]-mn lt 10) and (yBS[imin] - yBS[dim] lt 10) and (CHISQ gt 0)) and (Fore[ishock] eq 1) and (abs(yfit[0]-BU[ishock]) lt upDev) and (~pointerinup) and ~badsl and total(yfit) gt 0 and yfit[ishock-imin] gt 0
		...
		while 0 do begin
			...
			if ((onstep eq 1) or(yp[0] gt Mean([mx,mn])))  then begin
			 ...
			 ;if more than the width of previous, try fitting with m2=0 
				...
				;print,'status=',status
				working=1*(((status eq 0) and (MM[1] gt 0)) and (CHISQ lt minChi) and (bi-ishock gt minRemain) and ( MM[0] gt 1) and (MM[3]-MM[0]-mn lt 10) and (MM[1] gt minM1) and (CHISQ gt 0)) and (abs(yfit[0]-BU[ishock]) lt 4)
				if working then begin
...
;			dim=nDD[dloc];+doffsetb]
			...
			foot=(where(ofoots lt aj and ofoots ge dim))[0]
			t=ntau[aj]
...
;			BSh=nyBS[hhj]
			...
			aj=ajRefiner(dim,aj,bi,zeron,dobug,nyBS,nbeg,nend, nmono,nhill,nhill2,orderCustom,nextr,nrhod,nFore)


			;aj=aaj
			ab=(aj+bi+bi)/3
			while ab lt bi and max(nymm[aj:ab])+5 lt max(nymm[ab:bi],abmax) do bi=abmax+ab-1
			while max(nys[aj-60:aj]) gt max(nys[aj:bi]) do aj--
...
;			gxg=where(derv eq max(derv))
;			ishock=gxg[0]+imin
			working=1*(   ((status eq 0) and (MM[1] gt 0)) and (finite(CHISQ) eq 1) and (CHISQ lt minChi) and (imax-ishock gt minRemain) and ( MM[0] gt minM0) and (ishock gt imin) and (ishock lt imax) and (abs(MM[3]-MM[0]-mn) lt maxUp) and (MM[1] gt minM1) and (CHISQ gt 0)) and (nFore[ishock] eq 1) and (abs(yfit[0]-nBU[ishock]) lt upDev) and ~pointerinup; (TOTAL(nBmed[max([ishock-10,0]):min([ishock+2*60,imax])] gt MM[0]+MM[3]) ne 0)
			if doPerturb then begin
				working1=working
				yfit1=yfit
				MM1=MM
				ishock1=ishock
				CHI1=CHISQ
				imin=max([dim,foot-180]);8*t]);min([ishock-7*60,bfoot])])
				imax=min([bi,ishock+7*60])
				while nyBS[imin]-nyBS[dim] gt 2 do begin
				imin--
				;ishock--
				;imax--
				endwhile
				...
				;smypp=(smooth(ypp,2,/edge_mirror))[0:8*(ishock-imin)/10]
				if MM[3]-MM[0]  lt min(smypp)  or MM[3]-MM[0]  gt max(smypp) or total(MM[3]-MM[0] gt smypp) lt 5 or total(MM[3]-MM[0] lt smypp) lt 5 then begin;
				;if MM[3]-MM[0]  lt min(ypp[0:9*(ishock-imin)/10]) or MM[3]-MM[0]  gt max(ypp[0:9*(ishock-imin)/10]) or total(MM[3]-MM[0] gt ypp[0:9*(ishock-imin)/10]) lt 5 or total(MM[3]-MM[0] lt ypp[0:9*(ishock-imin)/10]) lt 5   then begin
					mx=MM[3]+MM[0]
 ...
 and (array_equal(hill2e[0:min([60,imax-ishock])], hill2e[SORT(hill2e[0:min([60,imax-ishock])])]) )) 
				;gxg=where(derv eq max(derv))
				;ishock=gxg[0]+imin
				working=1*(   ((status eq 0) and (MM[1] gt 0)) and (finite(CHISQ) eq 1) and (CHISQ lt minChi) and (bi-ishock gt minRemain) and ( MM[0] gt minM0) and (ishock gt imin) and (ishock lt imax) and (abs(MM[3]-MM[0]-mn) lt maxUp) and (MM[1] gt minM1) and (nyBS[imin] - nyBS[dim] lt 10) and (CHISQ gt 0)) and (nFore[ishock] eq 1) and (abs(yfit[0]-nBU[ishock]) lt upDev) and ~pointerinup;(TOTAL(nBmed[max([ishock-10,0]):min([ishock+2*60,imax])] gt MM[0]+MM[3]) ne 0)
			...
			;working=1*(   ((status eq 0) and (MM[1] gt 0)) and (finite(CHISQ) eq 1) and (CHISQ lt minChi) and (imax-ishock gt minRemain) and ( MM[0] gt 1) and (ishock gt imin) and (ishock lt imax) and (MM[3]-MM[0]-mn lt 10) and (MM[1] gt minM1) and (CHISQ gt 0))
...
;------------------------------END OF STANDARD RANGE--------------;------------------------------END OF STANDARD RANGE--------------;------------------------------END OF STANDARD RANGE--------------
	;print,'nz[ni9shock-3:ni9shock+3]=',nz[ni9shock-3:ni9shock+3]
		...
		if error_status eq 0 then begin
			...
			dim=bfoot;(bfoot*3+dim)/4;mean([bfoot,bfoot,bfoot,bfoot,bfoot,dim])
				...
				errorloc="a0=ajRefiner(dim,a0,bi,zeron,dobug,nyBS,nbeg,nend, nmono,nhill,nhill2,orderCustom,nextr,nrhod,nFore)"
				ishock=a0
				a0=ajRefiner(dim,a0,bi,zeron,dobug,nyBS,nbeg,nend, nmono,nhill,nhill2,orderCustom,nextr,nrhod,nFore)
				ab=(a0+bi+bi)/3
				while ab lt bi and max(nymm[a0:ab])+5 lt max(nymm[ab:bi],abmax) do bi=abmax+ab-1
				b0=bi
				while max(nys[a0-60:a0]) gt max(nys[a0:bi]) do a0--
				yp=nys[dim:bi]
				np=size(yp,/n_el)
				ishock=a0
				xp=nxs[dim:b0]
				...
				if ishock le dim then begin
					if m2/m1 gt 0 then ishock=m2/m1+dim else ishock=a0
				endif
				if ishock ge bi-10 then begin
					if m2/m1+dim lt bi then ishock=m2/m1+dim else ishock=a0
				endif
 ...
 and (array_equal(hill2e[0:min([60,imax-ishock])], hill2e[SORT(hill2e[0:min([60,imax-ishock])])]) )) 
				...
				working =1*( ((status eq 0) and (MM[1] gt 0)) and (finite(CHISQ) eq 1) and (CHISQ lt minChi) and (bi-ishock gt minRemain) and ( MM[0] gt 1) and (ishock gt imin) and (ishock lt imax)  and (abs(MM[3]-MM[0]-mn) lt maxUp) and (MM[1] gt minM1) and (nyBS[max([dim,ishock-7*60])] - nyBS[dim] lt 10) and (CHISQ gt 0)) and (nFore[ishock] eq 1) and (abs(yfit[0]-nBU[ishock]) lt upDev) and ~pointerinup;(TOTAL(nBmed[max([ishock-10,0]):min([ishock+2*60,imax])] gt MM[0]+MM[3]) ne 0)
				print,'[imin,ishock,imax]=',[imin,ishock,imax]
				;ishock=round(MM[2]/MM[1]+dim)
				if doPerturb then begin
					working1=working
					...
					mn=min(smmp)
;				if MM[3]-MM[0]  lt min(smypp) or MM[3]-MM[0]  gt max(smypp) or total(MM[3]-MM[0] gt smypp) lt 5 or total(MM[3]-MM[0] lt smypp) lt 5   then begin
				if MM[3]-MM[0]  lt min(smmp)  or MM[3]-MM[0]  gt max(smmp) or total(MM[3]-MM[0] gt smmp) lt 5 or total(MM[3]-MM[0] lt smmp) lt 5 then begin
					;if MM[3]-MM[0]  lt min(ypp[0:9*(ishock-imin)/10]) or MM[3]-MM[0]  gt max(ypp[0:9*(ishock-imin)/10]) or total(MM[3]-MM[0] gt ypp[0:9*(ishock-imin)/10]) lt 5 or total(MM[3]-MM[0] lt ypp[0:9*(ishock-imin)/10]) lt 5   then begin
					mx=MM[3]+MM[0]
				...
				if MM[3]-MM[0]  lt min(smypp)  or MM[3]-MM[0]  gt max(smypp) or total(MM[3]-MM[0] gt smypp) lt 5 or total(MM[3]-MM[0] lt smypp) lt 5 then begin
					...
					PRINT, 'Function parameters: ', MM
					if dobug gt 0 then print,'MM0[2]-MM2[2]=',m2-MM[2],', MM0[2]/MM0[1]-MM2[2]/MM2[1]=',m2/m1-MM[2]/MM[1],',MM1[2]/MM1[1]-MM2[2]/MM2[1]=',MM1[2]/MM1[1]-MM[2]/MM[1]
					errorloc="ishock=findShock(MM[2],MM[1],yfit,imin,1)"
					ishock=findShock(MM[2],MM[1],yfit,imin,1)
					;ishock=round(MM[2]/MM[1]+dim);m2) +dim
					;derv=dir(yfit,1,'mid')
		...
		while 0 do begin
				...
				if ((onstep eq 1) or(yp[0] gt Mean([mx,mn])))  then begin
				 ...
				 ;if more than the width of previous, try fitting with m2=0 
					...
					printed='unsaved'
					NNN=where(FINITE(yfit) eq 0, ncount, COMPLEMENT=B_C, NCOMPLEMENT=countnc)
					ishock=round(MM[2]/MM[1]);m2) +dim
;				working =1*(((status eq 0) and (MM[1] gt 0)) and (CHISQ lt minChi) and ( MM[0] gt 1))
					working =1*(((status eq 0) and (MM[1] gt 0)) and (CHISQ lt minChi) and (bi-ishock gt minRemain) and ( MM[0] gt 1) and (ishock gt imin) and (ishock lt imax) and (CHISQ gt 0)) and (nFore[ishock] eq 1) (TOTAL(nBmed[max([ishock-10,0]):min([ishock+2*60,imax])] gt MM[0]+MM[3]) ne 0)
					print,'[imin,ishock,imax]=',[imin,ishock,imax]
					if ishock lt imin then print, 'bad ishock: ishock = ',ishock,' lt ', imin,'= imin'
					if ishock gt imax then print, 'bad ishock: ishock = ',ishock,' gt ', imax,'= imax'
