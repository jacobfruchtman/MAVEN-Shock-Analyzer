...
function typeCheck0,kindThresh
	errorBuilder=[0,1]
	typ=-1
	print,kindThresh
...
function thrloop0, y,th,dw,typ,typi,typf,extras,cleans=cleans
...
function monotize20,y,t;returns an array with 1 where monotonically increasing and starts returning 0  when that stops being true until things start increasing again. ignores random spikes
...
pro threshFlag0, plt,t,kindThresh=kindThesh,mnt=mnt,newName=newName,dointerpolate=dointerpolate,interplot=interplot
	...
	if (typ ne 23) and (typ ne 25) then begin
		...
		store_data,'monof',data={x:datp.x,y:mnflag,ytitle:'mono-flag'}
		store_data,'bmonof',data={x:datp.x,y:bmnflag,ytitle:'bmonof'}
