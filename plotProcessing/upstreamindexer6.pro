...
;			print,"[dim,ed,imin,i,imax]=",[dim,ed,imin,i,imax]
			...
			dhalf=mean([dim,i])
			down=indowns[ishock]
			wherefoot=where(footO[imin:ishock] ne 0)

			if wherefoot eq -1 then foot=imin else 	foot=footO[wherefoot[-1]]
			imin=max([imin,foot-10*60,i-30*60])
 
			print,"[dim,ed,imin,foot,i,imax]=",[dim,ed,foot,imin,i,imax]
;			if dhalf gt i or imin gt dim then dhalf= imin
			if dhalf gt i  then dhalf= imin
			print,"[dim,ed,imin,foot,i,imax]=",[dim,ed,imin,foot,i,imax]
			...
			;imin=max([imin,footin-10*60,ishock-30*60])
