...
cRH1 = LEGEND(target=[p20,p21,medplot1,medplot2], font_size=4,transparency=100,orient=0,/relative,pos=[xl2,1],VERTICAL_SPACING=.005)
	...
	FOR KK=0,3 do begin
			...
			plotlist2.add,errorplot(10.^((mlow+mhigh)/2.0), medovershoot,errorbars,color=seasonColors[KK],name="Median "+seasonNames0[KK],/overplot,layout=[2,4,3],errorbar_color=seasonColors3[KK]);,pos=[0.1 ,0.35 ,0.44531250,0.59635417])
...
xsub=X[seasonsubIndices[KK]]
	...
	FOR KK=0,3 do begin
			...
			ysub=Y[seasonsubIndices[KK]]
			medliner,xsub,ysub,binsize,mlow,mhigh,medovershoot,errorbars
			plotlist3.add,errorplot((mlow+mhigh)/2.0, medovershoot,errorbars,color=seasonColors[KK],name="Median "+seasonNames0[KK],/overplot,layout=[2,4,5],errorbar_color=seasonColors3[KK])
...
;
	...
	p20=scatterplot(X,B2B1_Fit,xtitle='$M_{A}$',ytitle='$B_{downstream}/B_{upstream}$',$
						SYMBOL='dot', /SYM_FILLED,sym_size=.05, $
...
;	medplot2=errorplot((mlow+mhigh)/2.0, medovershoot,errorbars,color="red",name="Median RH",/over,layout=[2,4,2],ERRORBAR_THICK=.3,ERRORBAR_COLOR='peru');,pos= [0.59218750 ,0.695 ,0.94531250, 0.93046875])
	...
	medplot2.save,dire+'B2B1_varPlots5.png';,res=600
	print,'done'
	plotlist1[-1].close
end
