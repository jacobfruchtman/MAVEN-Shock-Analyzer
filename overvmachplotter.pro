...
pro overvmachplotter,islast=islast,currtime=currtime,REMOVENEGATIVES=REMOVENEGATIVES,RemoveBigFracM=RemoveBigFracM,$
	...
	if not keyword_set(maxAllowedFM) then maxAllowedFM=20 else killBigFM=1;10 else killBigFM=1
	...
	if not keyword_set(minMfms) then minMfms=1.;0;1;0;1.;.5;1.;0.;1.0
	if not keyword_set(betamaxallowed) then betamaxallowed=20;30;20;30;20.;9.;20.
...
;	pwibins=list()
	...
	inOvers=0
	thinOvers=0
	wrongBmax=0 ;;where the wrong Bmax is used. key: W
	nonlocals=0
	BadMultis=0
	badCrusts=0
	badN_SN=0
	badFoots=0
	badFit=0 ;;the fit is simply bad. Wrong shock location, wrong upstream, wrong downstream, downstream based on averaging overshoot and undershoot or whatever, etc. key: T (for tanh)
	badN_SNlst=list()
	whereBadSN=list()
	missedanomolies=0
	missedmanual=0
	...
	for i=0,numPlots-1 do begin
		...
		;if   t00 gt time_double('2019-10-23')+1 then break
		...
		for j=0,numel(dat.t)-1 do begin
			...
			if isqperp then totquasiperp++ else totquasipar++
			...
			if 1/MM[1] gt 300 and total(instruct.CharAt(0) eq ['I','R','V','L','A','C','O','Q']) eq 0 then begin;and  total(instruct.CharAt(0) eq ['U','D','M']) eq 1 then begin
				wherebigwidths.add,t
				bigwidths.add,1/MM[1]
...
;			if isqperp then totquasiperp++ else totquasipar++
			...
			if ~allowfix and total(instruct.CharAt(0) eq ['U','D','M']) ne 0 then begin
				...
				if instruct.strlen() eq 2 then begin
					...
					if instruct eq 'B'  then begin
						...
						badDefaults++
						badPoint=1
						if not keyword_set(keepPathologies) then continue
					endif
				endif else begin
						badManual++
						if not keyword_set(keepPathologies) then continue
			...
			if instruct.StartsWith('T')  then begin
				badManual++
				badFit++
				;badNAN=1
				badPoint=1
			...
			if instruct.StartsWith('%')  then begin
				badManual++
				badStep++
				;badNAN=1
				badPoint=1
			...
			if instruct.StartsWith('W')  then begin
				badManual++
				wrongBmax++
				;badNAN=1
				badPoint=1
			...
			if instruct.StartsWith('P')  then begin
				badManual++
				badPhenomena++
				;badNAN=1
				badPoint=1
			...
			if instruct.StartsWith('S')  then begin
				badManual++
				badSheath++
				;badNAN=1
				badPoint=1
			...
			if finite(N_e) ne 1 or N_e le 0 or N_p le 0 or N_e /N_p lt 1/15. or N_e /N_p gt 2. or T_rat lt 0  then begin
				;badMfms++
				;print,"badMfms,",mfms,xt
				;print,"badFMs"
				badDens++
			...
			downup=down/up
			if ((downup ge maxMagJump or downM/upM ge maxMagJump or (downup lt 1. and down-up lt 1.3)) and ~instruct.StartsWith('Y') ) OR downup eq 0  then begin
				badPoint=1
				badB2B1++
				if keyword_set(keepPathologies) then continue
			...
			if  instruct.StartsWith('A') and badPoint eq 0 and (  (bta lt 20 and bta gt 10 ) )   then smallAs.add,xt
			if instruct.StartsWith('A') and badPoint eq 0 then begin ;and not (Mfms lt 1 and Mfms gt 0 ) and not (bta lt 30 and bta gt 20 ) and not (FM lt 20 and FM gt 10  )   then begin
				badPoint=1
				missedanomolies++
			...
			downstdlst.add,dstd
			upstdlst.add,ustd
			thetaNVlst.add,dat.flowangle[j];thetaVN[j];flowangle[j]
			alt=SQRT(MSO[0]^2+MSO[1]^2+MSO[2]^2)
			if 0 then begin
			usMSO=transpose(dat.upstartpos[j,*])
			umMSO=transpose(dat.upmidpos[j,*])
	...
	;print, [badNANs,baddists,baddots,badFMs,badBmax,badLS,badMeasure,badDens,badMfms,badbeta,missedanomolies,BadManual,missedmanual,badCrusts,badMultis]
		print,"[badNANs,baddists,baddots,badFMs,badBmax,badLS,badMeasure,badMfms,badbeta,badDensity,badMMs]"
...
;
	...
	openU,1,'Documents/bigwidthlist.txt'
	
	for i=0,numel(bigWidths)-1 do begin
		ttt=wherebigwidths[i]
		wd=bigWidths[i]
		txtt=x2greg(ttt,/strformat) +' ; 1/MM[1] = '+strtrim(wd,2)
		printf,1,txtt
	...
	N_SN=N_AVG.toarray()
	if size(Buvec,/n_dim) ne 2 then return
	vumag=fltarr(N)
	for i=0,N-1 do vumag[i]=SQRT(TOTAL(vuvec_fine[i,*]^2))
	bumag=fltarr(N)
	for i=0,N-1 do bumag[i]=SQRT(TOTAL(Buvec[i,*]^2))
	...
	AlfCalc=upMs/Sqrt(!const.mp* 10.0^6 *N_p) /10.0^9
	...
	for i=0,N-1 do begin
		...
		Malfven[i]=abs(v_normal[i])/Alfven[i]
		Mfms_corr[i]=abs(v_normal[i])/fms_corr[i]
	endfor

	cosPitch=dotproduct(Vuvec_fine,Buvec)/abs(vumag*bumag)
	Pitch=acos(cosPitch)
	PitchD=Pitch*180/!pi
	BuN=dotproduct(N_SN,Buvec)
	...
	conY=0.0*conx
	conz=cony
	for i=0,N-1 do conY[i]=N_Conic_cyl[i,0]*cos(Azimuth[i])
	for i=0,N-1 do conZ[i]=N_Conic_cyl[i,0]*sin(Azimuth[i])
	n_conic=fltarr(N,3)
	NN=Sqrt(ConX^2+conY^2+conZ^2)
	for i=0,N-1 do n_conic[i,0]=conX[i]/NN[i]
	for i=0,N-1 do n_conic[i,1]=conY[i]/NN[i]
	for i=0,N-1 do n_conic[i,2]=conZ[i]/NN[i]
	betas=betas.toarray()
	...
	CspeedI=SQRT(( 5./3 *T_ion )/m_p   )*!const.c/1000
	VA=Alfven
	fmsIon=Sqrt( .5*  (VA^2+CspeedI^2+sqrt((VA^2+CspeedI^2)^2-4*VA^2*CSpeedI^2*(cos(thetaNBn)^2) ) ))
	MFMS_ion=v_normal/fmsIon
	for i=0,numel(B2B1_Fit)-1 do begin
			...
			RH_parameters,Mfms_ion[i],ThetaNBn[i],beta_ion[i],a,b,c,d,yy,delta
...
if 1 then begin
	...
	store_data,"Mfms_ion",data={x:t,y:Mfms_ion,ytitle:['$M_{fms}^{ion only}$'],YN:['$M_{fms}^{ion}$'], fn:["Mfms_ion"],  binsize:[1.],radian:[0],degree:[0]}
	...
	store_data,'Pproton',data={x:t,y:5. *N_p*T_ion/3,ytitle:'Proton Thermal Pressure $5/3 n_i k_B T_p$ [$eV cm^{-3}$]',YN:'Proton Pressure',fn:'P_p',binsize:2,radians:[0],degree:[0]} 
	store_data,'Pelectron',data={x:t,y:N_e*T_el,ytitle:'Electron Thermal Pressure $n_e k_B T_e$ [$eV cm^{-3}$]',YN:'Electron Pressure',fn:'P_e',binsize:2,radians:[0],degree:[0]} 
	...
	store_data,'Ls',data={x: t ,y: Ls ,ytitle:[ 'Ls [deg]' ], YN:[ 'Mars Solar Longitude' ], fn:[ 'Ls' ], binsize:[ 10. ], radian:[0],degree:[1]}
...
store_data,'lmaven',data={x: t ,y: l_MAVEN ,ytitle:[ 'Standoff Distance $\Delta$ [km]' ], YN:[ "Standoff Distance" ], fn:[ 'latus_rectum' ], binsize:[ 500. ], radian:[0],degree:[0]}
store_data,'lmavenNorm',data={x: t ,y: l_MAVEN/R_mars ,ytitle:[ 'Normalized Standoff Distance $\Delta/R_{Mars}$ [R_{Mars}]' ], YN:[ "Normalized Standoff Distance" ], fn:[ 'latus_rectum_normalized' ], binsize:[ .5 ], radian:[0],degree:[0]}
	...
	store_data,'SolDist',data={x: t ,y: SolDist ,ytitle:[ '$R_{orb}$ [km]' ], YN:[ 'Solar distance [km]' ], fn:[ 'solDist' ], binsize:[ .005*semimajor ], radian:[0],degree:[0]}
	store_data,'SolDistNorm',data={x: t ,y: SolDist/semimajor ,ytitle:[ '$R_{orbit} [Semimajor_{orbit}]$' ], YN:[ 'Solar distance' ], fn:[ 'solDistNormalized' ], binsize:[ .005 ], radian:[0],degree:[0]}
...
store_data,'overshootAmplitudeLowRes',data={x: t ,y: normalOverheight-(bmax*downfluctuation)/downs ,ytitle:[ '($B_{Max}(1-\sigma_{BD}/\langle B\rangle_D) - B^{Fit}_{Down}$)/$B^{Fit}_{Down}$' ], YN:[ "Normalized overshoot height" ], fn:[ "Normalized_OvershootHeight_lowRes" ], binsize:[ .5 ], radian:[0],degree:[0]}

	store_data,'overshootAmplitudeLowResM',data={x: t ,y: normalOverheightM-downfluctuation/downMs ,ytitle:[ '($B_{Max}(1-\sigma_{BD}/\langle B\rangle_D) - B^{Measured}_{Down}$)/$B^{Measured}_{Downstream}$' ], YN:[ "Normalized overshoot height (Measured)" ], fn:[ "Normalized_OvershootHeightMeasured_lowRes" ], binsize:[ .5 ], radian:[0],degree:[0]}
...
store_data,'B2B1_RH_ion',data={x: t ,y: B2B1_RH_i ,ytitle:[ '$(B_D/B_U)_{RH ion}$ ($\beta_i$ only)' ], YN:[ 'RH Predicted Magnetic Jump ' ], fn:[ "B2B1_RH_i" ], binsize:[ .5 ], radian:[0],degree:[0]}
	...
	store_data,'Vumag',data={x: t ,y: vumag ,ytitle:[ '$v^{ion}$ [km/s]' ], YN:[ 'Ion Speed' ], fn:[ 'VionMag' ], binsize:[ 50. ], radian:[0],degree:[0]}
	store_data,'Velocity_coarse',data={x: t ,y: Vuvec_coarse ,ytitle:[ '$v^{ion}$ [km/s]' ], YN:[ 'Ion Velocity' ], fn:[ 'Vion' ], binsize:[ 50. ], radian:[0],degree:[0]}
	store_data,'Velocity_fine',data={x: t ,y: Vuvec_fine ,ytitle:[ '$v^{ion}$ [km/s]' ], YN:[ 'Ion Velocity' ], fn:[ 'Vion' ], binsize:[ 50. ], radian:[0],degree:[0]}
	store_data,'Velocity_N',data={x: t ,y: v_normal ,ytitle:[ '$v^{ion}_{N}$ [km/s]' ], YN:[ 'Shockward ion Velocity' ], fn:[ 'VionN' ], binsize:[ 50. ], radian:[0],degree:[0]}
store_data,"ThetaNVn2",data={x:t,y:ThetaNVnd2,ytitle:['$\theta_{NV} [deg]$'],YN:['$\theta_{NV}$'], fn:['thetaNV2'],  binsize:[5.0],radian:[0],degree:[1]}
	store_data,"ThetaNV2",data={x:t,y:ThetaNV2,ytitle:['$\theta_{NV1}$ [rad]'],YN:['$\theta_{NV1}$'], fn:['thetaNV_Rad2'],  binsize:[5*!pi/180],radian:[1],degree:[0]}


	store_data,'Buvec',data={x:t,y:Buvec,ytitle:['$\bf B_u$ [nT]'],YN:'Upstream Magnetic Field',fn:'Buvec',binsize:1,radian:[0],degree:[0]}
	Pdyn=N_p*938.272 *10.^6 *vumag^2
	store_data,'P_ion_dyn1',data={x:t,y:Pdyn,ytitle:['$Ion Dynamic Pressure$ [eV/c^2 (km/s)^2]'],YN:'Ion Dynamic Pressure',fn:'Pdyn',binsize:100.,radian:[0],degree:[0]}
	...
	store_data,"Mflow",data={x:t,y:Mflow,ytitle:['$M_{flow}=V_{SW}/\sqrt{C_S^2+V_A^2}$'],YN:['$Flow Mach number$'], fn:["Mflow"],  binsize:[1.],radian:[0],degree:[0]}
	;Bux=fltarr(N)
	;for i=0,N-1 do Bux[i]=Buvec[i,0]
	;theta_BX=acos(Bux/upM)
	;theta_BXd=theta_BX*180./!pi
	;theta_BXnd=theta_BXd
	store_data,"coneangle_nd",data={x:t,y:coneanglend,ytitle:['$\theta_{B_1 X} [deg]$'],YN:['Cone Angle'], fn:['coneangle'],  binsize:[5.0],radian:[0],degree:[1]}
	store_data,"coneangle_rad",data={x:t,y:coneangle,ytitle:['$\theta_{B_1 X}$ [rad]'],YN:['Cone Angle'], fn:['coneangle_Rad'],  binsize:[5*!pi/180],radian:[1],degree:[0]}

	store_data,"clockangle_nd",data={x:t,y:clockanglend,ytitle:['$\cos^{-1}(B_z/\sqrt{B_y^2+B_z^2}) [deg]$'],YN:['Clock Angle'], fn:['clockangle'],  binsize:[5.0],radian:[0],degree:[1]}
	store_data,"clockangle_rad",data={x:t,y:clockangle,ytitle:['$\cos^{-1}(B_z/\sqrt{B_y^2+B_z^2})$ [rad]'],YN:['Clock Angle'], fn:['clockangle_Rad'],  binsize:[5*!pi/180],radian:[1],degree:[0]}


	store_data,"ThetaNBn2",data={x:t,y:ThetaNBnd2,ytitle:['$\theta_{NB} recalculated [deg]$'],YN:['$\theta_{NB}$ V2.0'], fn:['thetaNB2'],  binsize:[5.0],radian:[0],degree:[1]}
	;store_data,"ThetaBXn",data={x:t,y:Theta_BXnd,ytitle:['$\theta_{B_1 X} [deg]$'],YN:['$\theta_{B_1 X}$'], fn:['thetaBX'],  binsize:[5.0],radian:[0],degree:[1]}
	;store_data,"ThetaBX",data={x:t,y:Theta_BX,ytitle:['$\theta_{XB_1}$ [rad]'],YN:['$\theta_{XB_1}$'], fn:['thetaBX_Rad'],  binsize:[5*!pi/180],radian:[1],degree:[0]}
...
store_data,'RHO-0_MSO',data={x:t,y:sqrt(XMSO^2+YMSO^2),ytitle:'shock $\rho_0=\sqrt{X^2+Y^2}$ (MSO) [km]',YN:'Equatorial Distance',fn:'RHO-0mso',binsize:500,radian:[0],degree:[0]}
	...
	store_data,'SZA',data={x:t,y:SZA,ytitle:'SZA [rad]',YN:'Solar Zenith Angle',fn:'SZA_rad',binsize:(5*!pi/180),radian:[1],degree:[0]}
store_data,'SZAd',data={x:t,y:SZA*180./!pi,ytitle:'SZA [deg]',YN:'Solar Zenith Angle',fn:'SZA_deg',binsize:(5),radian:[0],degree:[1]}
	store_data,'lat',data={x:t,y:lat,ytitle:'Latitude MSO [rad]',YN:'Latitude',fn:'lat',degree:[0],radian:[1],vec:[0],binsize:[10*!pi/180]}
	
	

	Z_GEO=alt*cos(!pi/180*(90-lat))
	RHO0_GEO=alt*sin(!pi/180*(90-lat))
	...
	store_data,'fitWidth',data={x:t,y:fitwidths,ytitle:'tanh width $2\pi /m_1$ [sec]',YN:'Fit Width',fn:"fitwidth",binsize:[30],radian:[0],degree:[0]}
	store_data,'Bdvec',data={x:t,y:Bdvec,ytitle:['$\bf B_D$ [nT]'],YN:'\langle \bf B\rm\rangle_D',fn:'Bdvec',binsize:1,radian:[0],degree:[0],vec:[1]}
	store_data,'Vdvec',data={x: t ,y: Vdvec ,ytitle:[ '$\langle v^{ion}\rangle_D$ [km/s]' ], YN:[ '$\langle v^{ion}\rangle_D$' ], fn:[ 'Vdvec' ], binsize:[ 50. ], radian:[0],degree:[0],vec:[1]}
	store_data,'Bustd',data={x: t ,y: upstds ,ytitle:[ '$\sigma_{BU}$ [nT]' ], YN:[ '$\sigma_{BU}$' ], fn:[ 'Bustd' ], binsize:[ .25 ], radian:[0],degree:[0]}
	store_data,'Bdstd',data={x: t ,y: downstds ,ytitle:[ '$\sigma_{BD}$ [nT]' ], YN:[ '$\sigma_{BD}$' ], fn:[ 'Bdstd' ], binsize:[ .25 ], radian:[0],degree:[0]}
	Vux=fltarr(N)
	help,Vuvec_coarse
	for i=0,N-1 do Vux[i]=Vuvec_coarse[i,0]
	theta_VX=acos(Vux/vumag)
	theta_VXd=theta_VX*180./!pi
	theta_VXnd=90-abs(90-theta_VXd)
	store_data,"ThetaVXn",data={x:t,y:Theta_VXnd,ytitle:['$\theta_{V_1 X} [deg]$'],YN:['$\theta_{V_1 X}$'], fn:['thetaVX'],  binsize:[5.0],radian:[0],degree:[1]}
	store_data,"ThetaVX",data={x:t,y:Theta_VX,ytitle:['$\theta_{XV_1}$ [rad]'],YN:['$\theta_{XV_1}$'], fn:['thetaVX_Rad'],  binsize:[5*!pi/180],radian:[1],degree:[0]}
...
store_data,'dsDX',data={x:t,y:dsDX,ytitle:'distance between shock and downstream start (MSO) [km]',YN:'downstream displacement 1',fn:'dsDX',binsize:20,radian:[0],degree:[0]}
	...
	binsizes=[1.,1.,5.0,.25]
	openW,1,'Documents/wrongParametersList.txt'
	foreach el,whereWrongParams do printf,1,el
	close,1
	;wait,20
...
;	goodquasiperp=0.
	...
	print,'1-goodqpar/totqpar= 1-',goodquasipar,'/',totquasipar,'=1-',goodquasipar/totquasipar,'=',1.-goodquasipar/totquasipar
	print,'1-goodqperp/totqperp= 1-',goodquasiperp,'/',totquasiperp,'=1-',goodquasiperp/totquasiperp,'=',1.-goodquasiperp/totquasiperp

	foreach el,wheremissing do print,"missing savedpoint at "+el
	
	openu,1,'Documents/aggragatestats.txt',/append
	printf,1,'for [minMfms,betamaxallowed,maxallowedFM]=',[minMfms,betamaxallowed,maxallowedFM]
	printf,1,'~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~'
	
	printf,1,"goodpoint/points=",goodPoints,"/",points
	
	printf,1,"total spring:",numel(springIndices)
	printf,1,"total summer:",numel(summerIndices)
	printf,1,"total autumn:",numel(autumnIndices)
	printf,1,"total winter:",numel(winterIndices)
;print,"[badNANs,baddists,baddots,badFMs,badBmax,badLS,badMeasure,badDensity,badMfms,badbeta,missedAnomolies,BadManual,missedManual,badCrusts,badMultis]"
	;print, [badNANs,baddists,baddots,badFMs,badBmax,badLS,badMeasure,badDens,badMfms,badbeta,missedanomolies,BadManual,missedmanual,badCrusts,badMultis]
		printf,1,"[badNANs,baddists,baddots,badFMs,badBmax,badLS,badMeasure,badMfms,badbeta,badDensity,badMMs]"
...
;	goodquasiperp=0.
	...
	printf,1,'1-goodqpar/totqpar= 1-',goodquasipar,'/',totquasipar,'=1-',goodquasipar/totquasipar,'=',1.-goodquasipar/totquasipar
	printf,1,'1-goodqperp/totqperp= 1-',goodquasiperp,'/',totquasiperp,'=1-',goodquasiperp/totquasiperp,'=',1.-goodquasiperp/totquasiperp
	printf,1,'-------'
	foreach el,smallAs do printf,1,'anom at '+el
	printf,1,'=============================='
	close,1
	RETURN
